...
public class test_MouseSprite : MonoBehaviour {
	...
	// TODO for test
	public GameObject test_displayQuad;
	...
	{		
		...
		bone.transform.position = new Vector3(worldCord.topRight.x, worldCord.center.y, 0);
	...
	private void SetupBone_Tail(GameObject spriteMeshRootGO, GameObject boneRootGO)
	{
		...
		/// Test
		Texture2D test_displayTex = new Texture2D(binaryImage.cols(), binaryImage.rows());
		Utils.matToTexture2D(binaryImage, test_displayTex);
		test_displayQuad.GetComponent<Renderer>().material.mainTexture = test_displayTex;
		///
